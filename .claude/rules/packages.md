...
Custom package derivations and flake templates for rebellion.

## packages/

Custom package derivations exposed as `pkgs.rebellion.{name}`.
...
}:
{
  ...
  environment.systemPackages = [ pkgs.rebellion.my-script ];
}
```
...
# Or from GitHub
nix flake init -t github:khaneliman/rebellion#rust

# Preview template
nix flake show github:khaneliman/rebellion#templates.rust
```
...
cd $(mktemp -d)
nix flake init -t ~/rebellion#rust
nix develop
nix build
```
