...
```nix
rebellion.programs.terminal.shells.zsh.enable = true;
rebellion.programs.graphical.wms.hyprland.enable = true;
rebellion.suites.development.enable = true;
```

**When to create `rebellion.*` options:**
...
```nix
programs.zoxide = let
  inherit (config.rebellion.programs.terminal) shells;
in {
  enable = true;
...
Access system config via `osConfig` in the module-args:
...
1. **Explicit custom theme** (highest) - When user enables
   `rebellion.theme.catppuccin` or similar
...
nh darwin switch  # MacOS (applies system + home)
