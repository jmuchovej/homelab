...
GitOps tree for the K3s clusters, one per cluster keyed by full domain (`clusters/da.jm0.io`, `clusters/en.jm0.io`). A NixOS-managed `FluxInstance` (`modules/services/kubernetes/kubernetes.nix`) syncs this repo per cluster — its sync path is `kubernetes/clusters/${host.dc-domain}/flux`, a dir holding **only** the root `cluster-apps.ks.yaml` (so Flux actually applies it — a `kustomization.yaml` beside it would shadow it and silently drop its HelmRelease patch). That root reconciles `clusters/<domain>/apps`, whose selection fans out to the shared `core/` bundle plus the cluster's own apps.
...
- Don't add `postBuild.substituteFrom` for domain vars — the per-cluster `cluster-apps.ks.yaml` patch injects `${DATACENTER}`/`${DC_DOMAIN}`/`${DOMAIN}`/`${ST_NODE}` into every child's `postBuild.substitute`. Apps set `postBuild.substitute` only for their own literals (component parameters).
- **Node names are never written in the shared tree.** Pool paths (`warp/k8s/pvcs`, `impulse/syncthing`) are a portable convention and stay literal; the node that owns a pool is not, so it arrives as a per-cluster substitution or lives in `clusters/<domain>/config/`. `StorageClass`es are there for exactly this reason — `allowedTopologies` names a host. Their `zfs-localpv` chart stays in `core`, since the CSI driver itself is generic.
- Storage classes are named for the **medium**, not the pool: `zfs-ssd` (default) and `zfs-hdd`. A pool name is per-cluster; "this workload wants fast storage" is not, so an app that must not land on spinning disk says `zfs-ssd` and stays portable.
- A bare `zfs` class survives as a deprecated alias of `zfs-ssd`. Every PVC predating the split has `storageClassName: zfs` in its spec — set explicitly, or written in by the default-class admission controller — and that field is **immutable**. Editing the 5 manifests that name it explicitly makes Flux patch an immutable field and stall the app; deleting the class breaks expansion and any PVC recreate. Bound volumes are unaffected either way, so the alias just stays until a real migration retires it.
- **All classes are `reclaimPolicy: Delete`, and no PVC carries `prune: disabled`.** Deleting a PVC — including Flux pruning one because its manifest left the tree — destroys the ZFS dataset and everything on it. The `prune: disabled` guard is applied to CNPG `Cluster`s but to nothing else; there is no volsync (the S3 bucket exists, the deployment does not).
- `cluster-apps` `dependsOn` `cluster-config`. A PVC created while no default `StorageClass` exists is stamped `storageClassName: ""` **permanently** and never binds, so the classes must land before any app that claims one.
...
Given the `default` enforcement mode above, **an allow-all policy is not a no-op — it is strictly worse than no policy.** Selecting an endpoint flips it from _unenforced_ (the datapath skips the policy lookup and structurally cannot drop) to _enforced_, and Cilium expands `fromEntities: cluster` / `fromEndpoints: {}` into **one policy-map entry per identity**, never a wildcard. Ordinary pod churn rewrites that map, and packets arriving mid-rewrite are dropped as `Policy denied`. A `kube-system-allow-all` copied from biohazard cost ~1% of all cluster DNS this way (2026-08-02, deleted); the `cnpg-database` NetworkPolicy cost the CNPG operator's `:8000` probes the same way (2026-07-19, still disabled). Both presented as random intermittent failures, not as policy problems.

Before adding any policy:

1. **Does a pod need this grant today?** An unenforced pod already reaches everything — a policy can only ever _restrict_ it. "Permissive addition" is a contradiction here.
