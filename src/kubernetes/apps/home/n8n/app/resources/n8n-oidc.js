...
function getCookieSecret(context) {
  ...
  const baseKey =
    process.env.N8N_ENCRYPTION_KEY || process.env.OIDC_CLIENT_SECRET || "n8n-oidc-hook-secret";
  const hash = crypto
    .createHash("sha256")
      ...
      async function (server, n8nConfig) {
        ...
        app.get("/auth/oidc/callback", async (req, res) => {
          try {
            ...
            if (!code || !state) {
              return res.redirect(
                "/signin?error=" + encodeURIComponent("Missing authorization code or state"),
              );
            }
            ...
            if (!stateCookie || !nonceCookie) {
              return res.redirect(
                "/signin?error=" + encodeURIComponent("Missing state cookies - session expired"),
              );
            }
            ...
            if (!statePayload || statePayload.state !== state) {
              return res.redirect(
                "/signin?error=" + encodeURIComponent("Invalid state - possible CSRF attack"),
              );
            }
            ...
            if (tokens.id_token) {
              ...
              if (noncePayload && idTokenClaims.nonce !== noncePayload.nonce) {
                return res.redirect(
                  "/signin?error=" + encodeURIComponent("Invalid nonce - possible replay attack"),
                );
              }
            }
            ...
            if (!userInfo.email || !isValidEmail(userInfo.email)) {
              return res.redirect(
                "/signin?error=" + encodeURIComponent("No valid email in OIDC response"),
              );
            }
            ...
            if (!user) {
              return res.redirect(
                "/signin?error=" + encodeURIComponent("Failed to create or find user"),
              );
            }
            ...
            res.redirect(
              "/signin?error=" + encodeURIComponent("Authentication failed: " + error.message),
            );
          }
        });
