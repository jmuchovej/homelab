...
```nix
{
  rebellion.suites.common.enable = true;
...
All options follow `rebellion.{category}.{subcategory}.{option}` structure.

**Example:**

```nix
rebellion.programs.terminal.tools.claude-code.enable = true;
```
...
rebellion.user.theme = "catppuccin-mocha";
# Then use: config.rebellion.user.theme throughout
