...
- **Structure:** Markdown with YAML frontmatter defining arguments
...
- **What:** Reusable knowledge loaded on-demand by agents/commands
- **When to use:** Domain knowledge needed by multiple agents
- **Structure:** Markdown documentation and reference material

**Categories:**

- `anthropic/`: Official Anthropic skills (document-skills, mcp-builder, etc.)
- `rebellion/`: Project-specific patterns (config-layering, theming,
  module-layout)
...
```nix
# modules/common/ai-tools/agents/{category}/{name}.md
---
name: My Agent
...
```nix
# modules/common/ai-tools/commands/{category}/{name}.md
---
...
- Follow rebellion option naming: `rebellion.*`
- Use lib functions from `/src/lib/`
- Apply theme from `config.rebellion.user.theme`
```
