---
name: nix-validation
description: "Comprehensive Nix validation: treefmt-nix integration, statix/deadnix linting, flake checks, rebellion-specific patterns. Use when validating Nix code or setting up linting infrastructure."
---
...
## rebellion-Specific Validation

```bash
# Validate module options follow namespace
nix eval .#nixosModules.default --apply 'm: builtins.attrNames (m {} { lib = (import <nixpkgs> {}).lib; config = {}; pkgs = {}; }).options.rebellion or {}'

# Check home-manager module
nix build .#homeConfigurations.rebellion.activationPackage --dry-run

# Verify darwin configuration
nix build .#darwinConfigurations.rebellion.system --dry-run

# Test specific host
nix build .#nixosConfigurations.rebellion.config.system.build.toplevel --dry-run
```
