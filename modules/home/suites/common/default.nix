...
let
  inherit (lib) mkIf mkDefault mkEnableOption getExe';
...
in
{
  ...
  config = mkIf cfg.enable {
    home.shellAliases = {
      nixcfg = "nvim ~/Syncthing/${namespace}/flake.nix";
    };
    ...
    home.packages = (with pkgs; [
      ...
      jaq yq-go # JQ (rust), YQ (go), and TQ (go, from YQ)
      jqp jnv   # Do jq things, interactively
      ...
      gping           # replaces: `ping`
      fd              # replaces: find
      silver-searcher # replaces: ack -> grep
      procs           # replaces: ps
      # https://github.com/NixOS/nixpkgs/blob/nixos-24.11/pkgs/by-name/tr/trashy/package.nix#L39
      #! TODO not supported on macOS ;(
      # trashy          # replaces: rm
    ...
    home.sessionVariables = {
      ...
      DELTA_PAGER = "${ov-bin} -F";
      PAGER       = "${ov-bin} -F -H3";
      MANPAGER    = "${ov-bin} --section-delimiter '^[^\s]' --section-header";
    };
