...
The seed manifests live in `bootstrap/kubernetes/` (alongside `checks.nix`),
not under this aspect — `bootstrap/` is outside the flake's `import-tree
./modules` walk, so anything there is inert data and can never be picked up
